...
// ▼▼▼ 修改: 为 DeepSeek API 调用添加超时设置 (例如15秒) ▼▼▼
...
// ... (工具定义 availableTools 保持不变, 但在axios调用中增加超时) ...
...
const availableTools = { 
    "get_weibo_hot_search": async () => { 
        console.log("Executing get_weibo_hot_search tool."); 
        const rsshubUrl = 'https://rsshub.app/weibo/search/hot'; 
        try { 
            const response = await axios.get(`http://localhost:${process.env.PORT || 3000}/parse-rss?url=${encodeURIComponent(rsshubUrl)}`, { timeout: 8000 }); 
            if (!response.data || !response.data.items) { return JSON.stringify({ error: "Failed to parse Weibo hot search data." }); } 
            const hotSearches = response.data.items.map(item => ({ title: item.title, link: item.link, })); 
            return JSON.stringify(hotSearches); 
        } catch (error) { 
            console.error("Weibo hot search fetch error:", error); 
            return JSON.stringify({ error: "Sorry, failed to fetch Weibo hot search." }); 
        } 
    }, 
