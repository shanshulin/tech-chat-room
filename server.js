...
const deepseek = new OpenAI({ apiKey: process.env.DEEPSEEK_API_KEY, baseURL: 'https://api.deepseek.com/v1', timeout: 15 * 1000 });
...
const availableTools = { "get_weibo_hot_search": async () => { console.log("Executing get_weibo_hot_search tool."); const rsshubUrl = 'https://rsshub.app/weibo/search/hot'; try { const response = await axios.get(`http://localhost:${process.env.PORT || 3000}/parse-rss?url=${encodeURIComponent(rsshubUrl)}`, { timeout: 8000 }); if (!response.data || !response.data.items) { return JSON.stringify({ error: "Failed to parse Weibo hot search data." }); } const hotSearches = response.data.items.map(item => ({ title: item.title, link: item.link, })); return JSON.stringify(hotSearches); } catch (error) { console.error("Weibo hot search fetch error:", error); return JSON.stringify({ error: "Sorry, failed to fetch Weibo hot search." }); } }, "web_search": async ({ query }) => { console.log(`Executing Google web_search with query: "${query}"`); const apiKey = process.env.GOOGLE_SEARCH_API_KEY; const cx = process.env.GOOGLE_SEARCH_CX; const url = `https://www.googleapis.com/customsearch/v1?key=${apiKey}&cx=${cx}&q=${encodeURIComponent(query)}`; try { const response = await axios.get(url, { timeout: 8000 }); if (!response.data.items || response.data.items.length === 0) { return JSON.stringify({ "info": "No search results found." }); } const results = response.data.items.map(item => ({ title: item.title, snippet: item.snippet })); return JSON.stringify(results.slice(0, 5)); } catch (error) { console.error("Google Search API error:", error.message); return JSON.stringify({ error: `Google Search API failed. Reason: ${error.message}` }); } }, "get_current_time": async () => { console.log("Executing get_current_time tool."); return new Date().toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' }); } };
...
app.post('/api/ai-chat', async (req, res) => {
    ...
    try {
        ...
        if (use_network && message.tool_calls && Array.isArray(message.tool_calls) && message.tool_calls.length > 0) {
            ...
            // ▼▼▼ 修复: 创建一个干净的 message 对象，只包含必要的字段 ▼▼▼
            const cleanMessage = {
                role: message.role,
            ...
            history.push(cleanMessage); // 将清理过的 message 添加到历史记录
            // ▲▲▲ 修复结束 ▲▲▲
